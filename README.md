...
Este é um aplicativo Laravel desenvolvido no ambiente Windows.

## Pré-requisitos

Antes de executar este aplicativo, verifique se você tem os seguintes requisitos:
...
4. Crie um arquivo `.env` com base no arquivo `.env.example` fornecido.
5. Configure o banco de dados no arquivo `.env`.
6. Execute o comando `php artisan key:generate` para gerar a chave do aplicativo.
...
Este aplicativo é uma lista de usuários consumindo dados de uma API externa. Ao acessar `http://localhost:8000/users`, você verá a lista de usuários paginada em uma tabela.
